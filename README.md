...
홈 화면의 월간 검색량과 경쟁도는 네이버 검색광고 키워드 도구 API에서 조회합니다. 이 API는 실시간 검색어·일간 순위 API가 아니므로, 키워드 목록과 TOP 5는 편집 큐레이션이며 날짜가 바뀌어도 자동으로 변경되지 않습니다. Render 환경 변수에 다음 값을 등록해야 합니다.
